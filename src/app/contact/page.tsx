...
export default function Contact() {
  ...
  useEffect(() => {
    ...
    // Set up camera with fixed perspective and position
    ...
    // Example: position the camera at (x=8, y=6, z=16)
    camera.position.set(0, 2, 6);
    // Make the camera look at the center of the scene (origin)
    camera.lookAt(0, 0, 0);
    ...
    loader.load(
      ...
      (gltf: ModelLoadedEvent) => {
        ...
        // Center the model (your existing code)
        const box = new THREE.Box3().setFromObject(model);
        const center = box.getCenter(new THREE.Vector3());
        model.position.sub(center);

        // Here's where you can adjust the model position:
        // Move UP/DOWN by changing Y position
        model.position.y = -3; // Positive values move up, negative move down

        // Move LEFT/RIGHT by changing X position
        model.position.x = 0; // Negative values move left, positive move right

        // Move FORWARD/BACKWARD by changing Z position
        model.position.z = 0; // Negative values move forward, positive move backward
      ...
      <div className="relative z-10 min-h-screen pt-24 ">
        <div className="container mx-auto px-4">
          <motion.div
            ...
            className="max-w-4xl pb-20 mx-auto"
